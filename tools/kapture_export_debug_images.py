...
logger = logging.getLogger('debug_images')


def export_debug_images(
        kapture_path: str,  # noqa: C901
        output_path: str,
    ...
    :param output_path: path to the ply file to create
        ...
        with csv.get_all_tar_handlers(kapture_path) as tar_handlers:
            ...
            if _should_do('keypoints') and kapture_data.keypoints:
                ...
                logger.info('creating keypoints in 3D : '
                            f'{kapture.io.features.get_keypoints_fullpath(keypoints_type, output_path)}')
            ...
            if _should_do('depth') and kapture_data.records_depth:
                ...
                for depth_map_name, depth_map_filepath in tqdm(depth_records_filepaths.items(),
                                                               disable=logger.level >= logging.CRITICAL):
                    depth_png_filepath = path.join(output_path, f'depth_images/{depth_map_name}.png')
...
def export_debug_images_command_line() -> None:
    """
    Export images of depth and keypoints using the parameters given on the command line.
    """
    export_choices = {
        # cmd : help
...
if __name__ == '__main__':
    export_debug_images_command_line()
