...
# if you can't use symlinks, add this to the command: --image_transfer skip ; then run:
kapture_copy_RobotCar_Seasons_images.py -v info --image-dir ./RobotCar-Seasons/images --target ./kapture/RobotCar-Seasons-v2

# if you want to import v1 instead (deprecated), add this to the command: --v1
----

If you cannot use symlinks, use the following script to copy the images to the individual kapture folders
