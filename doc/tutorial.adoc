...
# tar_handlers.close() should be called before exiting the program
tar_handlers = csv.get_all_tar_handlers(dataset_path)
kapture_data = csv.kapture_from_dir(dataset_path, tar_handlers)
----
...
keypoints_filepaths = keypoints_to_filepaths(kapture_data.keypoints, keypoints_type, kapture_dirpath, tar_handlers)
